...
import { Partners, Timeline } from "../sections/Portoflio";
...
const ProjectImage = ({ images, name, type }) => {
  ...
  return (
    <div className="lg:w-2/5 relative">
      <div
        className={`relative h-64 lg:h-full bg-muted ${
          !hasImages ? "flex items-center justify-center" : ""
        }`}
      >
        {hasImages ? (
          <>
    ...
    >
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="lg:w-3/5 p-6 lg:p-8 flex flex-col">
          ...
          {/* align="left" — default, matches left-aligned card layout */}
...
function ProjectsPageInner() {
  ...
  const handleTypeChange = (type) => {
    setProjectType(type);
    router.replace(type === "ALL" ? "/projects" : `/projects?type=${type}`, {
      scroll: false,
    });
  };

  const getPageTitle = () =>
    projectType === "ALL" ? "Our Projects" : getProjectTypeLabel(projectType);
...
function ProjectsPageMeta() {
  ...
  const getMetaTitle = () =>
    currentType === "ALL"
      ...
      : `${getProjectTypeLabel(currentType)} | Our Portfolio`;
